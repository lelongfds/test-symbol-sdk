...
class BlockHttp extends Http_1.Http {
    ...
    constructor(url) {
        super(url);
        this.blockRoutesApi = new nem2_sdk_openapi_typescript_node_client_1.BlockRoutesApi(url);
    }
    /**
    ...
    getBlockByHeight(height) {
        return rxjs_1.from(this.blockRoutesApi.getBlockByHeight(height.toString())).pipe(operators_1.map(({ body }) => this.toBlockInfo(body)), operators_1.catchError((error) => rxjs_1.throwError(this.errorHandling(error))));
    }
    /**
    ...
    getBlockTransactions(height, queryParams) {
        return rxjs_1.from(this.blockRoutesApi.getBlockTransactions(height.toString(), this.queryParams(queryParams).pageSize, this.queryParams(queryParams).id, this.queryParams(queryParams).ordering))
            .pipe(operators_1.map(({ body }) => body.map((transactionDTO) => {
    ...
    getBlocksByHeightWithLimit(height, limit) {
        return rxjs_1.from(this.blockRoutesApi.getBlocksByHeightWithLimit(height.toString(), limit)).pipe(operators_1.map(({ body }) => body.map((blockDTO) => this.toBlockInfo(blockDTO))), operators_1.catchError((error) => rxjs_1.throwError(this.errorHandling(error))));
    }
    /**
    ...
    getMerkleTransaction(height, hash) {
        return rxjs_1.from(this.blockRoutesApi.getMerkleTransaction(height.toString(), hash)).pipe(operators_1.map(({ body }) => new MerkleProofInfo_1.MerkleProofInfo(body.merklePath.map((payload) => new MerklePathItem_1.MerklePathItem(payload.position, payload.hash)))), operators_1.catchError((error) => rxjs_1.throwError(this.errorHandling(error))));
    }
}
