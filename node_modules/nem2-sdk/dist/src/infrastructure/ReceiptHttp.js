...
class ReceiptHttp extends Http_1.Http {
    ...
    constructor(url, networkType) {
        super(url);
        this.receiptRoutesApi = new nem2_sdk_openapi_typescript_node_client_1.ReceiptRoutesApi(url);
    ...
    getMerkleReceipts(height, hash) {
        return rxjs_1.from(this.receiptRoutesApi.getMerkleReceipts(height.toString(), hash)).pipe(operators_1.map(({ body }) => new MerkleProofInfo_1.MerkleProofInfo(body.merklePath.map((payload) => new MerklePathItem_1.MerklePathItem(payload.position, payload.hash)))), operators_1.catchError((error) => rxjs_1.throwError(this.errorHandling(error))));
    }
    /**
    ...
    getBlockReceipts(height) {
        return this.networkTypeObservable.pipe(operators_1.mergeMap((networkType) => rxjs_1.from(this.receiptRoutesApi.getBlockReceipts(height.toString())).pipe(operators_1.map(({ body }) => CreateReceiptFromDTO_1.CreateStatementFromDTO(body, networkType)), operators_1.catchError((error) => rxjs_1.throwError(this.errorHandling(error))))));
    }
}
