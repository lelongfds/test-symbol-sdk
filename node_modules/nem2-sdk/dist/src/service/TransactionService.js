...
class TransactionService {
    ...
    announce(signedTransaction, listener) {
        return this.transactionRepository.announce(signedTransaction).pipe(operators_1.flatMap(() => listener.confirmed(signedTransaction.getSignerAddress(), signedTransaction.hash)));
    }
    /**
    ...
    announceAggregateBonded(signedTransaction, listener) {
        return this.transactionRepository.announceAggregateBonded(signedTransaction).pipe(operators_1.flatMap(() => listener.aggregateBondedAdded(signedTransaction.getSignerAddress(), signedTransaction.hash)));
    }
    /**
    ...
    resolvedFromReceipt(transaction, aggregateIndex) {
        return this.receiptRepository.getBlockReceipts(transaction.transactionInfo.height).pipe(operators_1.map((statement) => transaction.resolveAliases(statement, aggregateIndex)));
    }
}
