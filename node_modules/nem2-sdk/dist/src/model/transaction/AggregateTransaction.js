...
class AggregateTransaction extends Transaction_1.Transaction {
    ...
    static createFromPayload(payload) {
        ...
        const builder = type === TransactionType_1.TransactionType.AGGREGATE_COMPLETE ?
            catbuffer_1.AggregateCompleteTransactionBuilder.loadFromBinary(format_1.Convert.hexToUint8(payload)) :
            catbuffer_1.AggregateBondedTransactionBuilder.loadFromBinary(format_1.Convert.hexToUint8(payload));
        const innerTransactions = builder.getTransactions().map((t) => format_1.Convert.uint8ToHex(catbuffer_1.EmbeddedTransactionHelper.serialize(t)));
        ...
        return type === TransactionType_1.TransactionType.AGGREGATE_COMPLETE ?
            ...
            }), networkType, consignatures, new UInt64_1.UInt64(builder.fee.amount)) : AggregateTransaction.createBonded(Deadline_1.Deadline.createFromDTO(builder.deadline.timestamp), innerTransactions.map((transactionRaw) => {
            return CreateTransactionFromPayload_1.CreateTransactionFromPayload(transactionRaw, true);
    ...
    get size() {
        ...
        this.innerTransactions.forEach((transaction) => {
            ...
            const paddedTransactionByte = catbuffer_1.GeneratorUtils.concatTypedArrays(transactionByte, innerTransactionPadding);
    ...
    generateBytes() {
        ...
        const cosignatures = this.cosignatures.map((cosignature) => {
            ...
            return new catbuffer_1.CosignatureBuilder(new catbuffer_1.KeyDto(signerBytes), new catbuffer_1.SignatureDto(signatureBytes));
        });
        const transactionBuilder = this.type === TransactionType_1.TransactionType.AGGREGATE_COMPLETE ?
            new catbuffer_1.AggregateCompleteTransactionBuilder(new catbuffer_1.SignatureDto(signatureBuffer), new catbuffer_1.KeyDto(signerBuffer), this.versionToDTO(), this.networkType.valueOf(), this.type.valueOf(), new catbuffer_1.AmountDto(this.maxFee.toDTO()), new catbuffer_1.TimestampDto(this.deadline.toDTO()), new catbuffer_1.Hash256Dto(this.calculateInnerTransactionHash()), transactions, cosignatures) :
            new catbuffer_1.AggregateBondedTransactionBuilder(new catbuffer_1.SignatureDto(signatureBuffer), new catbuffer_1.KeyDto(signerBuffer), this.versionToDTO(), this.networkType.valueOf(), this.type.valueOf(), new catbuffer_1.AmountDto(this.maxFee.toDTO()), new catbuffer_1.TimestampDto(this.deadline.toDTO()), new catbuffer_1.Hash256Dto(this.calculateInnerTransactionHash()), transactions, cosignatures);
    ...
    resolveAliases(statement) {
        ...
        return DtoMapping_1.DtoMapping.assign(this, { innerTransactions: this.innerTransactions.map((tx) => tx.resolveAliases(statement, transactionInfo.index))
