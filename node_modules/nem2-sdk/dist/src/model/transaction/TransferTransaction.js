...
class TransferTransaction extends Transaction_1.Transaction {
    ...
    static createFromPayload(payload, isEmbedded = false) {
        const builder = isEmbedded ? catbuffer_1.EmbeddedTransferTransactionBuilder.loadFromBinary(format_1.Convert.hexToUint8(payload)) :
            catbuffer_1.TransferTransactionBuilder.loadFromBinary(format_1.Convert.hexToUint8(payload));
    ...
    getMessageBuffer() {
        ...
        const typeBuffer = catbuffer_1.GeneratorUtils.uintToBuffer(this.message.type, 1);
        return this.message.type === MessageType_1.MessageType.PersistentHarvestingDelegationMessage ?
            payloadBuffer : catbuffer_1.GeneratorUtils.concatTypedArrays(typeBuffer, payloadBuffer);
    }
    /**
    ...
    generateBytes() {
        ...
        const transactionBuilder = new catbuffer_1.TransferTransactionBuilder(new catbuffer_1.SignatureDto(signatureBuffer), new catbuffer_1.KeyDto(signerBuffer), this.versionToDTO(), this.networkType.valueOf(), TransactionType_1.TransactionType.TRANSFER.valueOf(), new catbuffer_1.AmountDto(this.maxFee.toDTO()), new catbuffer_1.TimestampDto(this.deadline.toDTO()), new catbuffer_1.UnresolvedAddressDto(UnresolvedMapping_1.UnresolvedMapping.toUnresolvedAddressBytes(this.recipientAddress, this.networkType)), this.sortMosaics().map((mosaic) => {
            return new catbuffer_1.UnresolvedMosaicBuilder(new catbuffer_1.UnresolvedMosaicIdDto(mosaic.id.id.toDTO()), new catbuffer_1.AmountDto(mosaic.amount.toDTO()));
    ...
    toEmbeddedTransaction() {
        return new catbuffer_1.EmbeddedTransferTransactionBuilder(new catbuffer_1.KeyDto(format_1.Convert.hexToUint8(this.signer.publicKey)), this.versionToDTO(), this.networkType.valueOf(), TransactionType_1.TransactionType.TRANSFER.valueOf(), new catbuffer_1.UnresolvedAddressDto(UnresolvedMapping_1.UnresolvedMapping.toUnresolvedAddressBytes(this.recipientAddress, this.networkType)), this.sortMosaics().map((mosaic) => {
            return new catbuffer_1.UnresolvedMosaicBuilder(new catbuffer_1.UnresolvedMosaicIdDto(mosaic.id.id.toDTO()), new catbuffer_1.AmountDto(mosaic.amount.toDTO()));
        }), this.getMessageBuffer());
    }
    /**
    ...
    resolveAliases(statement, aggregateTransactionIndex = 0) {
        ...
        return DtoMapping_1.DtoMapping.assign(this, { recipientAddress: statement.resolveAddress(this.recipientAddress, transactionInfo.height.toString(), transactionInfo.index, aggregateTransactionIndex),
