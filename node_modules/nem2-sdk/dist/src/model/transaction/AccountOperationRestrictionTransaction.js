...
class AccountOperationRestrictionTransaction extends Transaction_1.Transaction {
    ...
    static createFromPayload(payload, isEmbedded = false) {
        const builder = isEmbedded ? catbuffer_1.EmbeddedAccountOperationRestrictionTransactionBuilder.loadFromBinary(format_1.Convert.hexToUint8(payload)) :
            catbuffer_1.AccountOperationRestrictionTransactionBuilder.loadFromBinary(format_1.Convert.hexToUint8(payload));
        const signer = format_1.Convert.uint8ToHex(builder.getSignerPublicKey().key);
        const networkType = builder.getNetwork().valueOf();
        const transaction = AccountOperationRestrictionTransaction.create(isEmbedded ? Deadline_1.Deadline.create() : Deadline_1.Deadline.createFromDTO(builder.getDeadline().timestamp), builder.getRestrictionFlags().valueOf(), builder.getRestrictionAdditions(), builder.getRestrictionDeletions(), networkType, isEmbedded ? new UInt64_1.UInt64([0, 0]) : new UInt64_1.UInt64(builder.fee.amount));
    ...
    toEmbeddedTransaction() {
        return new catbuffer_1.EmbeddedAccountOperationRestrictionTransactionBuilder(new catbuffer_1.KeyDto(format_1.Convert.hexToUint8(this.signer.publicKey)), this.versionToDTO(), this.networkType.valueOf(), TransactionType_1.TransactionType.ACCOUNT_OPERATION_RESTRICTION.valueOf(), this.restrictionFlags.valueOf(), this.restrictionAdditions, this.restrictionDeletions);
    }
    /**
     * @internal
     * @returns {AccountOperationRestrictionTransaction}
