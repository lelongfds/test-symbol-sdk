...
class AccountMosaicRestrictionTransaction extends Transaction_1.Transaction {
    ...
    static createFromPayload(payload, isEmbedded = false) {
        const builder = isEmbedded ? catbuffer_1.EmbeddedAccountMosaicRestrictionTransactionBuilder.loadFromBinary(format_1.Convert.hexToUint8(payload)) :
            catbuffer_1.AccountMosaicRestrictionTransactionBuilder.loadFromBinary(format_1.Convert.hexToUint8(payload));
        const signerPublicKey = format_1.Convert.uint8ToHex(builder.getSignerPublicKey().key);
        const networkType = builder.getNetwork().valueOf();
        const transaction = AccountMosaicRestrictionTransaction.create(isEmbedded ? Deadline_1.Deadline.create() : Deadline_1.Deadline.createFromDTO(builder.getDeadline().timestamp), builder.getRestrictionFlags().valueOf(), builder.getRestrictionAdditions().map((addition) => {
    ...
    generateBytes() {
        ...
        const transactionBuilder = new catbuffer_1.AccountMosaicRestrictionTransactionBuilder(new catbuffer_1.SignatureDto(signatureBuffer), new catbuffer_1.KeyDto(signerBuffer), this.versionToDTO(), this.networkType.valueOf(), TransactionType_1.TransactionType.ACCOUNT_MOSAIC_RESTRICTION.valueOf(), new catbuffer_1.AmountDto(this.maxFee.toDTO()), new catbuffer_1.TimestampDto(this.deadline.toDTO()), this.restrictionFlags.valueOf(), this.restrictionAdditions.map((addition) => {
            return new catbuffer_1.UnresolvedMosaicIdDto(addition.id.toDTO());
        }), this.restrictionDeletions.map((deletion) => {
            return new catbuffer_1.UnresolvedMosaicIdDto(deletion.id.toDTO());
        }));
