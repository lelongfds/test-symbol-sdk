...
class MultisigAccountModificationTransaction extends Transaction_1.Transaction {
    ...
    static createFromPayload(payload, isEmbedded = false) {
        const builder = isEmbedded ? catbuffer_1.EmbeddedMultisigAccountModificationTransactionBuilder.loadFromBinary(format_1.Convert.hexToUint8(payload)) :
            catbuffer_1.MultisigAccountModificationTransactionBuilder.loadFromBinary(format_1.Convert.hexToUint8(payload));
    ...
    generateBytes() {
        ...
        const transactionBuilder = new catbuffer_1.MultisigAccountModificationTransactionBuilder(new catbuffer_1.SignatureDto(signatureBuffer), new catbuffer_1.KeyDto(signerBuffer), this.versionToDTO(), this.networkType.valueOf(), TransactionType_1.TransactionType.MULTISIG_ACCOUNT_MODIFICATION.valueOf(), new catbuffer_1.AmountDto(this.maxFee.toDTO()), new catbuffer_1.TimestampDto(this.deadline.toDTO()), this.minRemovalDelta, this.minApprovalDelta, this.publicKeyAdditions.map((addition) => {
            return new catbuffer_1.KeyDto(format_1.Convert.hexToUint8(addition.publicKey));
        }), this.publicKeyDeletions.map((deletion) => {
            return new catbuffer_1.KeyDto(format_1.Convert.hexToUint8(deletion.publicKey));
        }));
