...
class ResolutionStatement {
    ...
    generateHash(networkType) {
        const type = this.resolutionType === ResolutionType_1.ResolutionType.Address ? ReceiptType_1.ReceiptType.Address_Alias_Resolution
            : ReceiptType_1.ReceiptType.Mosaic_Alias_Resolution;
        const builder = this.resolutionType === ResolutionType_1.ResolutionType.Address ? new catbuffer_1.AddressResolutionStatementBuilder(ReceiptVersion_1.ReceiptVersion.RESOLUTION_STATEMENT, type.valueOf(), new catbuffer_1.UnresolvedAddressDto(UnresolvedMapping_1.UnresolvedMapping.toUnresolvedAddressBytes(this.unresolved, networkType)), this.resolutionEntries.map((entry) => new catbuffer_1.AddressResolutionEntryBuilder(new catbuffer_1.ReceiptSourceBuilder(entry.source.primaryId, entry.source.secondaryId), new catbuffer_1.AddressDto(RawAddress_1.RawAddress.stringToAddress(entry.resolved.plain()))))) : new catbuffer_1.MosaicResolutionStatementBuilder(ReceiptVersion_1.ReceiptVersion.RESOLUTION_STATEMENT, type.valueOf(), new catbuffer_1.UnresolvedMosaicIdDto(UInt64_1.UInt64.fromHex(this.unresolved.toHex()).toDTO()), this.resolutionEntries.map((entry) => new catbuffer_1.MosaicResolutionEntryBuilder(new catbuffer_1.ReceiptSourceBuilder(entry.source.primaryId, entry.source.secondaryId), new catbuffer_1.MosaicIdDto(entry.resolved.toDTO()))));
