...
class Http {
    ...
    createNetworkTypeObservable(networkType) {
        if (networkType && networkType instanceof rxjs_1.Observable) {
            ...
            return rxjs_1.from(new symbol_openapi_typescript_node_client_1.NodeRoutesApi(this.url).getNodeInfo()).pipe(operators_1.map(({ body }) => body.networkIdentifier), operators_1.catchError((error) => rxjs_1.throwError(this.errorHandling(error)))).pipe(operators_1.shareReplay(1));
        }
    }
    ...
    call(remoteCall, mapper) {
        return rxjs_1.from(remoteCall).pipe(operators_1.map(({ body }, index) => mapper(body, index)), operators_1.catchError((error) => rxjs_1.throwError(this.errorHandling(error))));
    }
}
exports.Http = Http;
