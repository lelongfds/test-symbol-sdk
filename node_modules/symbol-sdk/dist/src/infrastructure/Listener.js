...
class Listener {
    ...
    handleMessage(message, resolve) {
        if (message.uid) {
            ...
            this.messageSubject.next({
                channelName: ListenerChannelName.block, message: new BlockInfo_1.BlockInfo(message.meta.hash, message.meta.generationHash, message.meta.totalFee ? UInt64_1.UInt64.fromNumericString(message.meta.totalFee) : new UInt64_1.UInt64([0, 0]), message.meta.numTransactions, message.block.signature, PublicAccount_1.PublicAccount.createFromPublicKey(message.block.signerPublicKey, message.block.network), message.block.network, message.block.version, message.block.type, UInt64_1.UInt64.fromNumericString(message.block.height), UInt64_1.UInt64.fromNumericString(message.block.timestamp), UInt64_1.UInt64.fromNumericString(message.block.difficulty), message.block.feeMultiplier, message.block.previousBlockHash, message.block.blockTransactionsHash, message.block.blockReceiptsHash, message.block.stateHash, message.block.beneficiaryPublicKey),
            });
        }
        else if (message.code) {
            this.messageSubject.next({
                channelName: ListenerChannelName.status, message: new TransactionStatusError_1.TransactionStatusError(Address_1.Address.createFromEncoded(message.address), message.hash, message.code, Deadline_1.Deadline.createFromDTO(message.deadline)),
            });
        }
    ...
    transactionHasSignerOrReceptor(transaction, address) {
        if (address instanceof NamespaceId_1.NamespaceId) {
            return transaction instanceof TransferTransaction_1.TransferTransaction
                && transaction.recipientAddress.equals(address);
        }
